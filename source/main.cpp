...
void uart_event_handle()
{
	...
	if (uart0().read(&data_array[index], 1) == 1)
	{
		index++;
	}
...
{
	uart0().write(&p_data[0], length);
	static uint8_t newlines[3] = "\r\n";
	uart0().write(&newlines[0], sizeof(newlines));
...
int main()
{
    ...
    // The best solution is to start the OS before any other initialisation.
	...
	// Start execution.
    if (pdPASS != xTaskCreate(ble_stack_thread, "BLE", 256, NULL, 2, NULL))
    {
        APP_ERROR_HANDLER(NRF_ERROR_NO_MEM);
    }
